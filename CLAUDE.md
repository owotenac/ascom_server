...
Can also serve the Expo web client as static files for standalone deployment.
...
To create a standalone Windows executable that includes both server and web client:
...
This will:
1. Build the Expo web client into `server_ascom/web_dist/`
2. Package everything into `server_ascom/dist/astro_app.exe`

The exe is fully self-contained - just copy `astro_app.exe` to the target machine.

**Note**: The `.env` file with `ASTROMETRY_API_KEY` must be in the same directory as the exe.
...
- **main.py**: FastAPI application entry point with CORS middleware. Defines API routes:
  ...
  - `/` - serves static web client from `web_dist/` (if present)

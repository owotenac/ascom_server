"""
Fixed version of simbad_query_hipsaohdhr.py
Uses direct TAP queries to avoid Python 3.14 + Windows overflow bug.
"""
import pathlib
import tqdm
from astropy.table import Table, vstack

from py.utils_fixed import custom_simbad
...
for subdir, max_id, combined_path in zip(
    ...
    for batch in tqdm.tqdm(range(max_id // query_batch_size + 1), desc=f"Querying {subdir.name}"):
        batch_file = subdir / f"simbad_{subdir.name}_{str(batch)}.dat"
        if batch_file.exists():  # skip already completed batches
            continue
        ...
        if len(result) > 0:
            result.write(batch_file, format="ascii", overwrite=True)
